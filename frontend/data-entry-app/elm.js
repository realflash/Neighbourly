...
var author$project$Main$init = function (_n0) {
	return _Utils_Tuple2(
		{addresses: _List_Nil, blockId: '', status: ''},
...
var author$project$Main$toUrl = function (id) {
	return A3(
...
var author$project$Main$getAddressesForBlockId = function (id) {
	return A2(
		elm$http$Http$send,
		author$project$Main$LoadAddresses,
		A2(
			elm$http$Http$get,
			author$project$Main$toUrl(id),
...
var author$project$Main$httpErrorString = function (error) {
	switch (error.$) {
		...
		case 'BadStatus':
			...
			return 'Bad Http Status: ' + elm$core$Debug$toString(response.status.code);
		default:
			var message = error.a;
			var response = error.b;
			return 'Bad Http Payload: ' + (elm$core$Debug$toString(message) + (' (' + (elm$core$Debug$toString(response.status.code) + ')')));
	}
};
	...
	function (msg, model) {
		if (msg.$ === 'Change') {
			...
			return (elm$core$String$length(newBlockId) === 11) ? _Utils_Tuple2(
				_Utils_update(
					model,
					{addresses: _List_Nil, blockId: newBlockId, status: 'loading..'}),
				author$project$Main$getAddressesForBlockId(newBlockId)) : _Utils_Tuple2(
				_Utils_update(
					model,
					{
						addresses: _List_Nil,
						blockId: newBlockId,
						status: 'Invalid block ID - you need ' + (elm$core$Debug$toString(
							11 - elm$core$String$length(newBlockId)) + ' more characters')
					}),
				elm$core$Platform$Cmd$none);
		} else {
			var result = msg.a;
			if (result.$ === 'Ok') {
				if (!result.a.b) {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{addresses: _List_Nil, status: 'No block found for ID: ' + model.blockId}),
						elm$core$Platform$Cmd$none);
				} else {
					var newAddresses = result.a;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{addresses: newAddresses, status: ''}),
						elm$core$Platform$Cmd$none);
				}
			} else {
				var err = result.a;
...
var author$project$Main$viewAddress = function (address) {
	return A2(
		elm$html$Html$tr,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$text(address.street)
					]))
			]));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$Attributes$class('panel-half-relative panel-pad-sides padtopmore padbottommore logo-outer')
					]),
				_List_fromArray(
					[
						A2(
							...
							[
								...
								_List_fromArray(
									[
										...
										elm$html$Html$Attributes$class('text form-control'),
										A2(elm$html$Html$Attributes$attribute, 'maxLength', '11'),
										elm$html$Html$Attributes$pattern('^[0-9]{11}$'),
										elm$html$Html$Attributes$placeholder('Enter 11 digit Block ID'),
										A2(elm$html$Html$Attributes$attribute, 'required', 'true'),
										A2(elm$html$Html$Attributes$attribute, 'size', '11'),
										elm$html$Html$Attributes$title('Enter 11 digit Block ID'),
				...
				_List_fromArray(
					[
						elm$html$Html$text(model.blockId)
					])),
				A2(
				elm$html$Html$div,
				_List_Nil,
				...
				_List_fromArray(
					[
						...
						elm$html$Html$tbody,
						_List_Nil,
						A2(elm$core$List$map, author$project$Main$viewAddress, model.addresses))
					]))
			]));
};
