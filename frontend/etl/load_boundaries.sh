...
# Parse postgres:// URI into GDAL PG connection string
if [[ "$DB_URL" == postgres://* ]]; then
    ...
    OGR_CONN="PG:dbname='$PG_DB' host='$PG_HOST' port='$PG_PORT' user='$PG_USER' password='$PG_PASS'"
else
    OGR_CONN="PG:$DB_URL"
fi
...
ogr2ogr -f "PostgreSQL" "$OGR_CONN" "$SHAPEFILE" \
    -nln uk_output_areas \
    -nlt MULTIPOLYGON \
    -t_srs EPSG:4326 \
