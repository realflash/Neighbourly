...
- [x] Document the setup of the internal Node.js Spatial API pod and the necessary `LAMBDA_BASE_URL` routing.
- [x] Create `etl/load_boundaries.sh` to import ONS Output Area shapefiles into PostGIS.
- [x] Create `etl/transform_addresses.rb` to ingest a Sanitised Electoral CSV and the OUPRD, outputting SQL `COPY` statements for the addresses table.
- [x] Update the Node.js API to use WGS84 (SRID 4326) coordinate systems for UK compatibility.
